...
//Slightly modified vulnserver.c which requires a different amount of characters to buffer overflow than vulnserver.c
	...
	while (CSocket) {
		...
		if (Result > 0) {
			...
			if (strncmp(RecvBuf, "HELP", 4) == 0) {
				...
				char* notknown = "UNKNOWN COMMAND";
				strcat(RecvBuf, '\n');
				strcat(notknown, RecvBuf);
				SendResult = send(Client, notknown, 16, 0);
			}
			if (SendResult == SOCKET_ERROR) {
				printf("Send failed with error: %d\n", WSAGetLastError());
				closesocket(Client);
